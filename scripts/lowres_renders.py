...
if __name__ == '__main__':
    ...
    images = [f[len(args.path):] for f in sorted(glob.glob(os.path.join(args.path, "renders/", "*"))) if f.lower().endswith('png') or f.lower().endswith('jpg') or f.lower().endswith('jpeg')]
    ...
    for imagename in images:
        ...
        undis_img = cv2.resize(undis_img, (undis_img.shape[1] // 4, undis_img.shape[0] // 4), interpolation=cv2.INTER_AREA)
        ...
        print(os.path.join(os.path.join(args.path, "images"), imagename))
        cv2.imwrite(os.path.join(os.path.join(args.path, "images"), imagename), undis_img)
