...
def load_calib_and_compute_demo_intrinsics(
    ...
    if "calibrations" in calib:
        ...
        for cam_id, item in calib.items():
            ...
            D = np.array(item.get("distCoeff", [[0, 0, 0, 0, 0]])[0], dtype=np.float64).reshape(-1)
            W, H = map(int, item["imgSize"])
            R_w2c = np.array(item.get("R", np.eye(3)), dtype=np.float64)
...
def main():
    ...
    parser.add_argument("--crop_x", type=float, default=50)
    parser.add_argument("--crop_y", type=float, default=50)
    ...
    for img_path in image_files:
        ...
        frame = {
            ...
            "fl_x": float(K_demo[0, 0]) / 2,
            "fl_y": float(K_demo[1, 1]) / 2,
            "cx": float(K_demo[0, 2]) / 2,
            "cy": float(K_demo[1, 2]) / 2,
        ...
        if test_cam_key is not None and cam_key == test_cam_key or cam_key == "20114281" or cam_key == "22070928" or cam_key == "22053903":
