...
if __name__ == '__main__':
    ...
    #images = [f[len(args.path):] for f in sorted(glob.glob(os.path.join(args.path, "images_full_res/", "*"))) if f.lower().endswith('png') or f.lower().endswith('jpg') or f.lower().endswith('jpeg')]
    ...
    for k in cameras.keys():
        ...
        for imagename in images:
            if imagename[14:18] == k:
                ...
                undis_img = cv2.resize(undis_img, (undis_img.shape[1] // 3, undis_img.shape[0] // 3), interpolation=cv2.INTER_AREA)
                ...
                cv2.imwrite(os.path.join(os.path.join(args.path, "images_low_res"), imagename[19:]), undis_img)
