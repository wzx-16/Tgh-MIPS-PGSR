...
// Max Gaussian power over an axis-aligned tile rectangle (StopThePop-style
// tight tile culling): finds the in-rect point closest (in conic metric) to
// the mean; returns its opacity factor (0 if the mean lies inside the rect).
template<uint32_t PATCH_WIDTH, uint32_t PATCH_HEIGHT>
__device__ inline float max_contrib_power_rect_gaussian_float(
	const float4 co,
	const float2 mean,
	const float2 rect_min,
	const float2 rect_max,
	float2& max_pos)
{
	...
	dim3 grid)
{
	...
	if (radii[idx] > 0)
	{
		...
		// key/value pair IF the gaussian can actually contribute >= 1/255
		// somewhere inside that tile (tight per-tile culling). The key is
		// |  tile ID  |  depth  |; culled slots are padded with invalid
		// keys that sort to the very end of the list.
		for (int y = rect_min.y; y < rect_max.y; y++)
		{
			for (int x = rect_min.x; x < rect_max.x; x++)
			{
				const float2 tile_min = {(float)(x * BLOCK_X), (float)(y * BLOCK_Y)};
				const float2 tile_max = {(float)((x + 1) * BLOCK_X - 1), (float)((y + 1) * BLOCK_Y - 1)};
...
CudaRasterizer::SampleState CudaRasterizer::SampleState::fromChunk(char*& chunk, size_t C)
{
	...
	obtain(chunk, sample.all_map_sampled, NUM_ALL_MAP * C * BLOCK_SIZE, 128);
	return sample;
}
	...
	bool debug)
{
	...
	// Count buckets (groups of 32 sorted gaussians) per tile and allocate
	// the checkpoint state consumed by the accelerated backward.
	...
	CHECK_CUDA(FORWARD::render(
		...
		sampleState.T, sampleState.ar, sampleState.all_map_sampled,
		width, height,
		focal_x, focal_y,
		cx, cy,
		viewmatrix,
	...
	// The accelerated backward reconstructs per-pixel suffix state from the
	// final pixel color (including background), so keep a copy.
