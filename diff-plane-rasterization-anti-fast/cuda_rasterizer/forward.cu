...
renderCUDA(
	...
	float* __restrict__ sampled_all_map,
	int W, int H,
	...
	const bool render_geo)
{
	...
	// Bucket bookkeeping for the accelerated backward: this tile owns
	// ceil(toDo/32) buckets starting at per_tile_bucket_offset[tile_id-1].
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		{
			// Checkpoint the running state for every 32nd gaussian so the
			// backward can restart blending from any bucket boundary.
			if (j % 32 == 0) {
				...
				if (render_geo) {
					for (int ch = 0; ch < ALL_MAP; ++ch) {
						sampled_all_map[(bbm * BLOCK_SIZE * ALL_MAP) + ch * BLOCK_SIZE + block.thread_rank()] = All_map[ch];
					}
				}
				++bbm;
			}
	...
	if (inside)
	{
		...
		if (render_geo) {
			...
			out_plane_depth[pix_id] = All_map[4] / -(All_map[0] * ray.x + All_map[1] * ray.y + All_map[2] - 1.0e-6);
		}
	}

	// Per-tile max of last_contributor: buckets entirely beyond it are
	// skipped by the accelerated backward.
...
void FORWARD::render(
	...
	float* sampled_T, float* sampled_ar, float* sampled_all_map,
	int W, int H,
	const float focal_x, const float focal_y,
	const float cx, const float cy,
	const float* viewmatrix,
