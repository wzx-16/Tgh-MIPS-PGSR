...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int, gaussian_dim : int = 3, time_duration: list = [-0.5, 0.5], rot_4d: bool = False, force_sh_3d: bool = False, sh_degree_t : int = 0, current_timestamp : float = 0.0, device: str = "cuda"):
        ...
        self.rot_4d = rot_4d
        self._rotation_r = torch.empty(0, device=device)
    ...
    def clear_gaussians(self, gaussians):
        ...
        self._rotation_r = torch.empty(0, 4, device=self.device)
    ...
    def create_from_multi_pcd(self, path, tgh, spatial_lr_scale : float, time_duration=None):
        ...
        for ii, pcd_path in enumerate(pcd_list[:]):
            ...
            if pcd.points.shape[0] > 30000:
                mask = np.random.randint(0, pcd.points.shape[0], 30000)
                xyz = pcd.points[mask]
                rgb = pcd.colors[mask]
            ...
            if self.gaussian_dim == 4:
                ...
                if self.rot_4d:
                    rotation_r = torch.zeros((fused_point_cloud.shape[0], 4), device=self.device)
                    rotation_r[..., 0] = 1.
            ...
            if self.gaussian_dim == 4:
                ...
                self._scaling_t = nn.Parameter(scales_t.requires_grad_(True))
                if self.rot_4d:
                    self._rotation_r = nn.Parameter(rotation_r.requires_grad_(True))
        ...
        pcd = fetchPly(ply_path)
        if pcd.points.shape[0] > 1:
            mask = np.random.randint(0, pcd.points.shape[0], 1)
            xyz = pcd.points[mask]
            rgb = pcd.colors[mask]
            normals = pcd.normals[mask]
        ...
        if self.gaussian_dim == 4:
            ...
            if self.rot_4d:
                self._rotation_r = nn.Parameter(_rotation_r.requires_grad_(True))
